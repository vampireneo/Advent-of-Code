...
const countArrangement = (springs, groups) => {
    while (true) {
        ...
        if (springs.startsWith('?')) return countArrangementWithCache(`.${springs.slice(1)}`, groups)
        ...
        if (groups.length > 1) {
            if (springs.length < groups[0] + 1 || springs[groups[0]] === '#') return 0
...
const part1Answer = data.map(r => countArrangementWithCache(r.springs, r.groups))
    .reduce((p, c) => p + c, 0)

console.log(part1Answer)

const part2Answer = data.map(r => countArrangementWithCache(Array(5).fill(r.springs).join('?'), Array(5).fill(r.groups).flat()))
    .reduce((p, c) => p + c, 0)
